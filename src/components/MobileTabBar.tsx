...
import { LayoutDashboard, Calendar, Stethoscope, Settings, BarChart2 } from 'lucide-react'; // Importar BarChart2
...
const MobileTabBar: React.FC = () => {
  ...
  const tabs = [
    ...
    // Botón Central Destacado CON TEXTO
    { path: '/consultation', icon: Stethoscope, label: 'Consulta IA', isMain: true },
    { path: '/reports', icon: BarChart2, label: 'Reportes' }, // NUEVO
    { path: '/patients', icon: Settings, label: 'Pacientes' },
  ];

  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 pb-safe-area-inset-bottom shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-50">
        ...
        {tabs.map((tab) => (
          ...
          >
            {({ isActive }) => (
                <>
                    {tab.isMain ? (
                        // Diseño especial para botón central
                        <div className="absolute -top-5 flex flex-col items-center">
                             ...
                             <span className="text-[10px] font-bold text-brand-teal mt-1">Consulta IA</span>
                        </div>
                    ) : (
                        // Diseño normal
                        <>
