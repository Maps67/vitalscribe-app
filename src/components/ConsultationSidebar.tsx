    ...
    <div className={`w-full md:w-1/4 p-4 flex flex-col gap-2 border-r dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden h-full ${generatedNote ? 'hidden md:flex' : 'flex'}`}>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar flex flex-col gap-2 pr-1">
          ...
          {activeMedicalContext && !generatedNote && (
            <div 
              className="relative z-30 group shrink-0"
      ...
      `}>
        ...
        {selectedPatient && !(selectedPatient as any).isTemporary && (
          <button 
            ...
            className={`w-full mt-1 py-3 bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-bold rounded-xl shadow-lg shadow-indigo-200 dark:shadow-none hover:shadow-xl hover:scale-[1.02] transition-all items-center justify-center gap-2 group shrink-0
            ${(vitalSnapshot && isMobileSnapshotVisible) ? 'hidden md:flex' : 'flex'}`}
          >
