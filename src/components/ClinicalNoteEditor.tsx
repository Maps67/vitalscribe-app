...
const SPECIALTIES_WITH_CONTROLLED_RX = [
    'Psiquiatría', 'Neurología', 'Medicina Interna', 'Anestesiología', 
    'Algología', 'Cuidados Paliativos', 'Oncología Médica', 'Cirugía Oncológica'
];
       ...
       <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-slate-100 dark:bg-slate-950">
           <div className="min-h-full flex flex-col max-w-4xl mx-auto w-full gap-4 relative pb-8">
               ...
               {activeTab==='record' && generatedNote.soapData && (
                   <div className="bg-white dark:bg-slate-900 rounded-sm shadow-lg border border-slate-200 dark:border-slate-800 p-6 md:p-12 min-h-full h-fit pb-32 animate-fade-in-up relative">
                       
                       {/* HEADER CORREGIDO: Uso de Grid para evitar desbordamiento lateral */}
                       <div className="sticky top-0 z-20 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm border-b border-slate-100 dark:border-slate-800 pb-4 mb-8 -mx-2 px-2">
                           <div className="grid grid-cols-1 md:flex md:justify-between md:items-start gap-4">
                               ...
                               {/* Grupo de Acciones: Se apila en móvil, se alinea a la derecha en desktop */}
                               <div className="flex flex-wrap md:flex-col items-center md:items-end gap-2 justify-between md:justify-start">
                                   
                                   {/* Blindaje Legal */}
                                   <span className="w-full md:w-auto text-[10px] uppercase tracking-wider font-bold text-slate-400 flex items-center gap-1 mb-1 md:mb-0">
                                       <ShieldCheck size={12} className="text-brand-teal shrink-0"/> 
                                       <span className="truncate">IA: Borrador sujeto a revisión</span>
                                   </span>

                                   {/* Fila de Botones: Calculadora, Eliminar y Guardar */}
                                   <div className="flex items-center gap-2 w-full md:w-auto justify-end">
                                       {/* Botón Eliminar (Placeholder basado en tu imagen) */}
                                       <button className="p-2.5 bg-red-50 text-red-500 rounded-xl hover:bg-red-100 border border-red-100 transition-colors">
                                           <Trash2 size={20} />
                                       </button>

                                       {/* Botón Calculadora (El que causaba el desplazamiento) */}
                                       <button className="p-2.5 bg-slate-50 text-slate-600 rounded-xl hover:bg-slate-100 border border-slate-200 transition-colors">
                                           <HeartPulse size={20} />
                                       </button>
                                       ...
                                       <button 
                                           ...
                                           disabled={isSaving} 
                                           className="flex-1 md:flex-none bg-brand-teal text-white px-4 md:px-6 py-2.5 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-teal-600 shadow-md transition-all disabled:opacity-50"
                                       >
                       ...
                       <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800">
                                   ...
                                   {setSpecialFolio && showControlledInput && (
                                       <div className="flex-1 md:flex-none animate-fade-in-right">
                                           <input 
                                               ...
                                               className="text-xs border-2 border-indigo-100 dark:border-indigo-900/50 bg-indigo-50/50 dark:bg-indigo-900/20 rounded-lg px-3 py-1.5 w-full md:w-56 outline-none focus:border-indigo-500 text-indigo-700 dark:text-indigo-300 font-medium transition-all"
                                               value={specialFolio || ''}
                           ...
                           <div className="space-y-3">
                              ...
                              {editablePrescriptions.map((med, idx) => (
                                      <div key={idx} className="flex gap-2 items-start p-3 rounded-lg border group bg-slate-50 border-slate-100 dark:bg-slate-800/50 dark:border-slate-700">
                                          <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-2">
                                              <input className="w-full font-bold bg-transparent outline-none border-b border-transparent focus:border-indigo-300 text-slate-800 dark:text-white" value={med.drug} onChange={e=>onUpdateMedication(idx,'drug',e.target.value)} />
                                              <input className="text-sm bg-transparent outline-none border-b border-transparent focus:border-indigo-300 text-slate-600 dark:text-slate-300" value={med.dose} onChange={e=>onUpdateMedication(idx,'dose',e.target.value)} />
                                              <div className="col-span-2 flex gap-2 text-xs">
                                                  <input className="flex-1 bg-transparent outline-none border-b border-transparent focus:border-indigo-300 text-slate-500" value={med.frequency} onChange={e=>onUpdateMedication(idx,'frequency',e.target.value)} />
                                                  <input className="flex-1 bg-transparent outline-none border-b border-transparent focus:border-indigo-300 text-slate-500" value={med.duration} onChange={e=>onUpdateMedication(idx,'duration',e.target.value)} />
                                              </div>
                                          </div>
