...
import { useSpeechRecognition } from '../hooks/useSpeechRecognition'; // Importamos el hook de voz aquí también
import { GeminiMedicalService } from '../services/GeminiMedicalService'; // Y el servicio
...
const PatientsView: React.FC = () => {
  ...
  const [isRxModalOpen, setIsRxModalOpen] = useState(false); // Nueva Receta (Voz)
  ...
  // --- LOGICA RECETA DE VOZ ---
  const { isListening, transcript, startListening, stopListening } = useSpeechRecognition();
  const [rxText, setRxText] = useState(''); // Texto final de la receta
  ...
  const handleGenerateRx = async () => {
      ...
      try {
          // Usamos el nuevo modo "Solo Receta"
          ...
          stopListening(); // Asegurar que pare
      }
  };
  ...
  const handleSaveRx = async () => {
      ...
      try {
        ...
        // Guardamos como una "Consulta" pero especial
        // En 'transcript' guardamos lo que dictó el doctor
        // En 'summary' guardamos la receta formateada
        await supabase.from('consultations').insert([{
            doctor_id: user.id,
            patient_id: selectedPatient.id,
            transcript: "DICTADO DE RECETA: " + transcript, 
            summary: rxText, // Aquí va el texto bonito para el PDF
            status: 'completed'
        }]);

        setIsRxModalOpen(false);
        fetchHistory(selectedPatient.id); // Recargar lista
  ...
  const handleSharePDF = async (consultation: Consultation) => {
    ...
    try {
      ...
      } else { alert("Tu navegador no soporta compartir. Usa imprimir."); }
    } catch (error) { console.log("Cancelado"); } finally { setIsSharingId(null); }
  };
    ...
    return (
      <div className="p-6 max-w-6xl mx-auto animate-fade-in-up">
        ...
        <div className="flex justify-between items-center mb-6">
            ...
            {/* BOTÓN NUEVA RECETA (EL PROTAGONISTA) */}
            <button 
                onClick={handleOpenRxModal}
                className="bg-slate-900 text-white px-5 py-2.5 rounded-lg font-bold shadow-lg flex items-center gap-2 hover:bg-slate-800 transition-transform active:scale-95"
            >
            ...
            {history.map((consultation) => (
                <div key={consultation.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden group">
                    ...
                    <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex flex-wrap justify-end gap-3">
                        ...
                        <button onClick={() => handleSharePDF(consultation)} disabled={isSharingId === consultation.id} className="bg-brand-teal text-white px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-2 hover:bg-teal-600 transition-colors shadow-sm">
                            {isSharingId === consultation.id ? <RefreshCw size={16} className="animate-spin"/> : <Share2 size={16}/>} Compartir
                        </button>
                        <PDFDownloadLink document={<PrescriptionPDF doctorName={doctorProfile.full_name} specialty={doctorProfile.specialty} license={doctorProfile.license_number} phone={doctorProfile.phone} university={doctorProfile.university} address={doctorProfile.address} logoUrl={doctorProfile.logo_url} signatureUrl={doctorProfile.signature_url} patientName={selectedPatient.name} date={new Date(consultation.created_at).toLocaleDateString()} content={consultation.summary || ""} />} fileName={`Receta.pdf`} className="bg-slate-800 text-white px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-2 hover:bg-slate-700 shadow-sm">
                    ...
                    <div className="flex-1 p-6 overflow-y-auto bg-slate-50/50">
                        ...
                        {!rxText ? (
                            <div className="flex flex-col items-center justify-center h-full space-y-6 py-10">
                                ...
                                {transcript && (
                                    <div className="w-full bg-white p-4 rounded-xl border border-slate-200 text-sm text-slate-600 italic">
                                        "{transcript}"
                                    </div>
                                )}
  ...
  // ... resto del código (vista lista) ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      {/* ... (Misma vista de lista de antes) ... */}
