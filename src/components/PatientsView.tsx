...
import { 
  ...
  Stethoscope, Trash2, Share2, Download, Paperclip, FolderOpen, History
} from 'lucide-react';
      ...
      <div className={`w-full md:w-1/3 p-4 flex flex-col gap-4 border-r dark:border-slate-800 bg-white dark:bg-slate-900 overflow-y-auto ${generatedNote ? 'hidden md:flex' : 'flex'}`}>
        <div className="flex justify-between items-center mb-1">
            <h2 className="text-2xl font-bold dark:text-white flex items-center gap-2">
                Consulta IA 
                {/* BOTÓN CLIP: Solo habilitado si hay paciente seleccionado */}
                <button 
                    onClick={() => selectedPatient ? setIsAttachmentsOpen(true) : toast.info("Seleccione un paciente para ver sus archivos")}
                    className={`p-2 ml-1 rounded-full transition-colors ${selectedPatient ? 'bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 text-brand-teal' : 'text-slate-300 cursor-not-allowed'}`}
                ...
                <div className="flex justify-between items-center mb-4 pb-2 border-b dark:border-slate-800">
                    <div>
                        <h3 className="font-bold text-lg dark:text-white flex items-center gap-2">
                            <FolderOpen size={20} className="text-brand-teal"/> Expediente
                        </h3>
